...
$k=$dao->buscaExaustiva($field,'utilizador u','u._id_administracao=?',$param);
$i=0;
?>
        ...
        <div class="col-md-12 col-sm-6">
            <div class="widget widget-stats bg-blue">
                ...
                <div class="stats-info">
                    <p><i class="fa fa-list"></i> Listar Utilizador</p>
                    <h4>&nbsp;</h4>
                </div>
            </div>
        </div>
                ...
                <div class="panel-body">
                    <table id="data-table" class="table table-striped table-bordered">
                        ...
                        <?php foreach ($k as $rs):
                            ...
                            $funcao=$userDao->listarPorId('grupo',$rs->_id_grupo)->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($funcao as $funcaors)
                            {
                                $nomeFunc=$funcaors['_perfil'];
                            }

                            $morad=$userDao->listarPorId('morada',$rs->_morada)->fetchObject();
                            $muni=$userDao->listarPorId('municipio',$morad->_id_municipio)->fetchObject();
                            $bairro=$userDao->listarPorId('bairro',$morad->_id_bairro)->fetchObject();
                            $morada=$muni->_nome.' - '.$bairro->_nome;

                            if($userDao->listarPorId('conta',$rs->_id)->rowCount()==0)
                            {
